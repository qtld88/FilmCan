...
| G5 | **Purgeable / no-fail space.** On a drive where Finder shows ample free but `df -h` (statfs) shows little (i.e. mostly local snapshots), run a copy larger than the statfs-free figure. | FilmCan does NOT false-block; before writing it thins local snapshots (`tmutil listlocalsnapshots /` shows fewer after); the copy COMPLETES without ENOSPC. If it genuinely cannot fit even after reclaim, it blocks cleanly up front (never a mid-write ENOSPC). | ☐ |
| G6 | **Green verify bar steady.** Watch the destination card on a Fast-mode run, then a Paranoid run. | Fast: yellow copy bar only, then a solid completion check — NO green bar flashing in/out. Paranoid: green verify overlay advances steadily, never blinking. | ☐ |
